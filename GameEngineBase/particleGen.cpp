...
void ParticleEmitter::SetAcceleration( Vector3D newAcceleration )
{
...
void ParticleEmitter::SetLocation( Vector3D newLocation )
{
...
void ParticleEmitter::GenerateParticles( 
		int numParticles, Vector3D initVelocity,
...
void ParticleEmitter::GetParticles( std::vector< CParticle > &vecLiveParticles )
{
	...
	for ( int index = 0; index != this->m_myParticles.size(); index++ )
	{
		...
		{
		//Dead
			aliveParticles =false;
		}
	}
	return;
}

void ParticleEmitter::m_CreateParticle(CParticle &tempParticle)
{
	tempParticle.age = getRand( this->m_startingAge / 2.0f, 
			                    this->m_startingAge );	
