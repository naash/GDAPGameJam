...
{   
	...
	//degrees  
	steering.m_maxSteeringAngle = 45 * ( HK_REAL_PI / 180 );// previously 35 and steering was very to and fro 

	// [mph/h] The steering angle decreases linearly 
	// based on your overall max speed of the vehicle. 
	steering.m_maxSpeedFullSteeringAngle = 25.0f * (1.605f / 3.6f);
...
{
	engine.m_maxTorque =  1000.0f;// previously 100 and going full here and there
...
void VehicleSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultTransmission& transmission )
{
	...
	const hkReal wheelRadius = 0.05f;
...
void VehicleSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultSuspension& suspension ) 
{
	...
	//
	{
		const hkReal hardPointFrontX = 0.25f;
		const hkReal hardPointBackX = -0.25f;
		const hkReal hardPointY = 0.018f;
